...
class Graph(Scene):
    def construct(self):
        ...
        stem_1 = Line(
            node_B.get_right(), [-4.5, 0, 0], color=text_black, stroke_width=2.5
        )
        ...
        arrow_7 = make_arrow(
            start=node_A, end=node_C, label="", scale_label=0.4, label_shift=0.3
        ).shift(LEFT * 0.1 + UP * 0.03)

        arrow_8 = make_arrow(
            start=node_C, end=node_A, label="", scale_label=0.4, label_shift=0.3
        ).shift(RIGHT * 0.1 + DOWN * 0.05)

        stem_2 = Line(
            node_A.get_right(), [-1.5, 0, 0], color=text_black, stroke_width=2.5
        )

        stem_3 = Line(
            node_C.get_right(), [-1.5, 0, 0], color=text_black, stroke_width=2.5
        )
        ...
        all_stems = VGroup(stem_1, stem_2, stem_3)

        # Curved Arrow

        arrow_10 = CurvedArrow(
            start_point=node_B.get_top() + DOWN * 0.7,
            end_point=node_D.get_top() + DOWN * 0.5,
            radius=-3.1,
            color=text_black,
        ...
        labels_axes = axes.get_axis_labels(
            MathTex("X", color=text_black, stroke_width=1.5).scale(0.7),
            MathTex(
                rf"f(x) \in \mathbb{{R}}", color=text_black, stroke_width=1.5
            ).scale(0.7),
        )
        ...
        for x, label in zip(x_positions, labels):
            text = (
                MathTex(rf"\mathbf{{{label}}}", font_size=60)
                .scale(0.6)
        ...
        line_AB = DashedLine(
            point_A, point_B, dash_length=0.15, color=text_black, stroke_width=3
        )
        line_BC = DashedLine(
            point_B, point_C, dash_length=0.15, color=text_black, stroke_width=3
        )
        line_CD = DashedLine(
            point_C, point_D, dash_length=0.15, color=text_black, stroke_width=3
        )

        all_nodes = VGroup(node_A, node_B, node_C, node_D)

        all_arrows = VGroup(*[locals()[f"arrow_{i}"] for i in range(1, 11)])
