...
class Graph(Scene):
    def make_node(self, position, label="x", radius=0.3, node_color="#e1c180", text_color="#22323b"):
        ...
        label = MathTex(label, color = text_color, stroke_color = text_color, stroke_width = 1, font_size = 70).move_to(position)
    ...
    def make_arrow(self, start, end, label="x", label_shift=0.2, color="#22323b", tip_shape = StealthTip):
        ...
        arrow = Arrow(start = start, end = end, color = color, tip_shape = tip_shape, stroke_width = 2.5, buff = 0, tip_length = 0.04)

        angle = arrow.get_angle()

        #label_arrow = Text(label, weight = BOLD, color = color).next_to(arrow.get_center())
        label_arrow = MathTex(label, color = color, stroke_color = color, stroke_width = 1, font_size = 70).move_to(arrow.point_from_proportion(0.5)).rotate(angle).shift(UP * label_shift)
    ...
    def make_curved_arrow(self, node, color, label):
        ...
        label_arrow = MathTex(label, color = color, stroke_color = color, stroke_width = 1, font_size = 70).move_to(carrow.point_from_proportion(0.5)).shift( RIGHT * 0.2)
