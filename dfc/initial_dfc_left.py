...
Fig Caption: Left: Initial policy DFC

Generate image by simple running script:
python ./initial_dfc_left.py
"""
...
class Graph(Scene):
    ...
    # Nodes 
        a_left= make_node(position=[-5, 0, 0], label="a", node_color = node_orange)

        a_top = make_node(position=[-2,2,0], label="a", node_color = node_orange) 

        a_bot = make_node(position=[-2,-2,0], label="a", node_color = node_orange)

        b_right = make_node(position=[0,0,0], label="b", node_color= node_pink) 

        b_right_right = make_node(position=[4,0,0], label="b", node_color= node_pink) 
    ...
    # Label
        image_label = MathTex("D", color = text_black, stroke_color = text_black, stroke_width = 1.3, font_size = 80).move_to([5,2,0])
        ...
        all_objects = VGroup(all_arrows, all_nodes, triangle, image_label)
