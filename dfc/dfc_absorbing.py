...
class Graph(Scene):
    ...
    # Nodes 
        c_left = make_node(position=[-5, 0, 0], label="c", node_color = node_blue)

        a_top = make_node(position=[-2,1,0], label="a", node_color = node_orange) 

        b_bot = make_node(position=[-2,-1,0], label="b", node_color = node_pink)

    # Arrows

        arrow_1 = make_arrow(start = c_left, end = a_top, label = "0", scale_label = 0.4, label_shift = 0.3)

        arrow_2 = make_arrow(start = c_left, end = b_bot, label = "1", scale_label = 0.4, label_shift = 0.3)

        #arrow_3 = make_curved_arrow_top(start_node = a_top, end_node = a_top, color = text_black, label = "0,1", radius = 0.21, label_scale = 0.4).shift(0.03 * RIGHT)

        arrow_3 = make_self_loop_top(radius = 0.3, start_angle = 11*PI/6, angle = 5*PI/4, stroke_width = 3, start_node = a_top, color = text_black, label = "0,1", label_scale = 0.4, label_shift = UP * 0.05).shift(0.03 * RIGHT + UP * 0.13)

        #arrow_4 = make_curved_arrow_top(start_node = b_bot, end_node = b_bot, color = text_black, label = "0,1", radius = 0.21, label_scale = 0.4).shift(0.03 * RIGHT)

        arrow_4 = make_self_loop_top(radius = 0.3, start_angle = 11*PI/6, angle = 5*PI/4, stroke_width = 3, start_node = b_bot, color = text_black, label = "0,1", label_scale = 0.4, label_shift = 0.05).shift(0.03 * RIGHT + UP * 0.13)
    ...
    # Triangle 
        ...
        all_nodes = VGroup(c_left,b_bot, a_top)
